...
function curry2 <Def extends Def2, X extends SecondParameter<Def>>(executor: Def, _:__, x: X): <Y extends FirstParameter <Def>>(y: Y) => ReturnType<Def>;
...
function curry2 <Def extends Def2>(executor: Def): {
  ...
  <X extends SecondParameter<Def>>(_:__ ,x: X): <Y extends FirstParameter<Def>>(y: Y) => ReturnType<Def>;
};


function curry2 <Y, X, R>(executor: Def2<Y, X, R>, y?: Y | X |__, x?: X) {
		switch (arguments.length) {
				case 1: return function (y: Y | X |__, x?: X) {
						switch (arguments.length) {

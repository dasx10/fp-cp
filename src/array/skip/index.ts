...
type InputKey <Key extends number, X extends readonly any[]> = number extends Key ? Key : (Index<X> & Key);
...
function skip <X extends readonly any[]> (_: placeholder, x: X): <Key extends number>(index: number extends Key ? Key : (Index<X> & Key)) => ArraySkip<Key, X>;
...
function skip <Key extends number, X extends readonly any[]>(index: Key | placeholder, array?: X | Key) {
  ...
  else if (index === _) return <Key extends Index<X>>(index: Key) => index < 0 ? _skipRight(<Key>index, <X>array) : _skipLeft(<Key>index, <X>array);
  return index < 0 ? _skipRight(<Key>index, <X>array) : _skipLeft(<Key>index, <X>array);
}

export default Object.assign(skip, {
  core  : _skip,
  right : skipRight,
  left  : skipLeft,
});

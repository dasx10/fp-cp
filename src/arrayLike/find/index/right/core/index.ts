...
const arrayLikeFindIndexRightCore: ArrayLikeFindIndexRightCore = <X extends ArrayLike<unknown>>(def: (value: ArrayLikeValue<X>, index: ArrayLikeIndex<X>, arrayLike: X) => any, x: X): ArrayLikeIndex<X> | -1 => {
  const { length } = x;
  if (length > 0) {
    let index = length;

...
const arrayLikeFindEntryRightCore: ArrayLikeFindEntryRightCore = <X extends ArrayLike<unknown>>(def: (value: ArrayLikeValue<X>, index: ArrayLikeIndex<X>, arrayLike: X) => any, x: X) => {
	const { length } = x;
	switch (length) {
		case 0 : return;
		case 1 : {
